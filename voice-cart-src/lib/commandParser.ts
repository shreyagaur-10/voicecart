...
export function parseCommand(transcript: string): ParsedIntent {
  ...
  if (removeMatch) {
    ...
    // Also handle: "remove X from the list/cart"
  ...
  if (addMatch) {
    ...
    if (!itemName) return { action: 'unknown', transcript: raw };
    return {
      action: 'add',
      item: itemName,
      quantity,
  ...
  // Treat short phrases as add commands: "milk", "2 apples", "eggs"
  const { quantity, rest, unit } = parseQuantity(raw);
  const itemName = cleanItemName(rest);
  if (itemName && itemName.split(' ').length <= 4 && !raw.includes('?')) {
    return {
      action: 'add',
      item: itemName,
      quantity,
